...
factor <- 11
factor.cov.dates <- read.csv(file = paste("MOFA/3.downstream_analysis/",groups,"_",samples,"/prediction/factor",factor,"_cov_dates.csv", sep = ""))
...
coxRegression <- function(biomarker) {
  ...
  write.csv(factor.cov.dates, file = paste("MOFA/3.downstream_analysis/",groups,"_",samples,"/prediction/factor",factor,"_cov_dates.csv", sep = ""),
            row.names = FALSE)
...
write.table(Cstat.results,
            file=paste("MOFA/3.downstream_analysis/",groups,"_",samples,"/prediction/Cstat_results.csv", sep = ""),
            col.names = T,
            row.names = T,
            sep=",")

...
setwd("/home/guille/Guille/MBHS/2Y/TFM/MOFA")

groups <- "nogroups"
samples <- "914_2055"
factor <- 23
#factor <- 11
factor.cov.dates <- read.csv(paste(groups,"_",samples,"/prediction/factor",factor,"_cov_dates.csv", sep = ""), sep = ",", stringsAsFactors = FALSE)
head(factor.cov.dates)
dim(factor.cov.dates)

# comprobar -1.01226
factor.cov.dates[factor.cov.dates$shareid == "10011", "factor"]
...
## probar con factor 12 categorica n 2 grupos
...
cat("\n\nResults for model 2: factor12 + sex + age\n\n", file = stdout())
mod.surv.cvd.2

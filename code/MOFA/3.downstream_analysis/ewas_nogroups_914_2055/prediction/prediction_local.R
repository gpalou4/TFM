...
CoxRegression <- function(factor, sex = "All", outcome) {
  ...
  if (outcome == "chd") {
    ...
    factor.cov.dates <- factor.cov.dates[!((factor.cov.dates$cvd == 1) & (factor.cov.dates$chd == 0)),]
  }
  ...
  coxph.exp.mod1 <- paste0("coxph(Surv(",outcome,"followup, ",outcome,") ~ factor + NK + CD8T + CD4T + Bcell + Mono + Gran + sva1_METH, data = factor.cov.dates)")
  
  mod.surv.cvd.1 <- eval(parse(text=coxph.exp.mod1))
    
  #cat("\n\nResults for model 1: factor + cells + sva1\n\n", file = stdout())
  mod.surv.cvd.1
  ...
  if (sex == "All") {
    coxph.exp.mod2 <- paste0("coxph(Surv(",outcome,"followup, ",outcome,") ~ factor + age + sex + NK + CD8T + CD4T + Bcell + Mono + Gran + sva1_METH, data = factor.cov.dates)")
    coxph.exp.mod3 <- paste0("coxph(Surv(",outcome,"followup, ",outcome,") ~ factor + age + sex + tot_chol + hdl_chol + sbp + dbp + glucose + smoke + NK + CD8T + CD4T + Bcell + Mono + Gran + sva1_METH, data = factor.cov.dates)")
  }
  else if ((sex == "Male") | (sex == "Female")) {
    coxph.exp.mod2 <- paste0("coxph(Surv(",outcome,"followup, ",outcome,") ~ factor + age + NK + CD8T + CD4T + Bcell + Mono + Gran + sva1_METH, data = factor.cov.dates)")
    coxph.exp.mod3 <- paste0("coxph(Surv(",outcome,"followup, ",outcome,") ~ factor + age + tot_chol + hdl_chol + sbp + dbp + glucose + smoke + NK + CD8T + CD4T + Bcell + Mono + Gran + sva1_METH, data = factor.cov.dates)")
    }
  
  mod.surv.cvd.2 <- eval(parse(text=coxph.exp.mod2))
  
  #cat("\n\nResults for model 2: factor + sex + age + cells + sva1\n\n", file = stdout())
  mod.surv.cvd.2
  
  mod.surv.cvd.2.df <- as.data.frame(summary(mod.surv.cvd.2)$coefficients)
  mod.surv.cvd.2.df
  mod.surv.cvd.2.df$lower95 <- as.numeric(exp(confint(mod.surv.cvd.2)))[1:dim(mod.surv.cvd.2.df)[1]] #[1:3]
  mod.surv.cvd.2.df$upper95 <- as.numeric(exp(confint(mod.surv.cvd.2)))[(dim(mod.surv.cvd.2.df)[1]+1):(dim(mod.surv.cvd.2.df)[1]*2)] #[4:6]
  ...
  mod.surv.cvd.3 <- eval(parse(text=coxph.exp.mod3))
  
  #cat("\n\nResults for model 3: factor + sex + age + tot_chol + hdl_chol + sbp + dbp + smoke + glucose + cells + sva1\n\n", file = stdout())
  mod.surv.cvd.3
  ...
  cox.res.all <- rbind(mod.surv.cvd.1.df,mod.surv.cvd.2.df,mod.surv.cvd.3.df)
  cox.res.factors <- cox.res.all[grep("factor",rownames(cox.res.all)),]
  rownames(cox.res.factors) <- NULL
  return(cox.res.factors)
  
}

outcome <- "chd"
cox.res.factors.all <- c()
for (factor in 1:30) {
  print(paste("FACTOR ",factor,sep = ""))
  if (factor == 21) {
    cox.res.factors.1 <- CoxRegression(factor = factor, sex = "Male", outcome = outcome)
    cox.res.factors.2 <- CoxRegression(factor = factor, sex = "Female", outcome = outcome)
    cox.res.factors.all <- rbind(cox.res.factors.all,cox.res.factors.1,cox.res.factors.2)
    next
  }
  cox.res.factors <- CoxRegression(factor = factor, outcome = outcome)
  cox.res.factors.all <- rbind(cox.res.factors.all,cox.res.factors)
}

# Looking at interaction sex*factor --> CVD sig Factor 21 (0.00669452); CHD sig Factor 20 (0.003143258)

cox.res.factors.all$P_adjusted <- NA

# FDR adjust by model
cox.res.factors.all[cox.res.factors.all$model == "Model 1",]$P_adjusted <- p.adjust(cox.res.factors.all[cox.res.factors.all$model == "Model 1",]$P, method = "fdr")
cox.res.factors.all[cox.res.factors.all$model == "Model 2",]$P_adjusted <- p.adjust(cox.res.factors.all[cox.res.factors.all$model == "Model 2",]$P, method = "fdr")
cox.res.factors.all[cox.res.factors.all$model == "Model 3",]$P_adjusted <- p.adjust(cox.res.factors.all[cox.res.factors.all$model == "Model 3",]$P, method = "fdr")
cox.res.factors.all

write.csv(cox.res.factors.all, file = paste("MOFA/3.downstream_analysis/",groups,"_",samples,"/prediction/cox_results_",outcome,".csv", sep = ""),row.names = FALSE)

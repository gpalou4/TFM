...
samples_metadata(MOFA.model) <- MOFA.covariates.chd[-2]
...
# 
# # p-values FDR adjust
# all.factors.res$t_test_adj <- p.adjust(all.factors.res$t_test, method = "fdr")
# all.factors.res$m_whitney_wilcoxon_adj <- p.adjust(all.factors.res$m_whitney_wilcoxon, method = "fdr")
# 
...
cat("\n\n... X.1) STATISTICAL TESTS BETWEEN INDIVIDUALS WITH CHD AND NO CHD AND NO CVD ... \n\n", file = stdout())

# Remove CVD individuals with no CHD

factors.chd.cvd.filt <- factors$group1[!((MOFA.model@samples_metadata$cvd == 1) & (MOFA.model@samples_metadata$chd == 0)),]
MOFA.covariates.chd.cvd.filt <- MOFA.model@samples_metadata[!((MOFA.model@samples_metadata$cvd == 1) & (MOFA.model@samples_metadata$chd == 0)),]

all.factors.res <- c()
# for (i in 1:30) {
#   if (i == 21) {next}
#   test.factor.res <- testFactor(factors = factors.chd.cvd.filt, factor = i, covariable = "chd", group = "all", MOFA_samples_metadata = MOFA.covariates.chd.cvd.filt)
#   all.factors.res <- rbind(all.factors.res, test.factor.res)
# }

# Factor 21 stratified by sex
# Male

MOFA.covariates.chd.male <- MOFA.covariates.chd.cvd.filt[MOFA.covariates.chd.cvd.filt$sex == 1,]
head(MOFA.covariates.chd.male,n = 15)
factors.chd.male <- factors.chd.cvd.filt[rownames(factors.chd.cvd.filt)%in%MOFA.covariates.chd.male$sample,]
head(factors.chd.male, n=15)

test.factor.res <- testFactor(factors = factors.chd.male, factor = 21, covariable = "chd", group = "male", MOFA_samples_metadata = MOFA.covariates.chd.male)
all.factors.res <- rbind(all.factors.res, test.factor.res)

# Female
MOFA.covariates.chd.female <- MOFA.covariates.chd.cvd.filt[MOFA.covariates.chd.cvd.filt$sex == 2,]
head(MOFA.covariates.chd.female,n = 15)
factors.chd.female <- factors.chd.cvd.filt[rownames(factors.chd.cvd.filt)%in%MOFA.covariates.chd.female$sample,]
head(factors.chd.female, n=15)

test.factor.res <- testFactor(factors = factors.chd.female, factor = 21, covariable = "chd", group = "female", MOFA_samples_metadata = MOFA.covariates.chd.female)
all.factors.res <- rbind(all.factors.res, test.factor.res)
...
write.csv(all.factors.res, file = paste("MOFA/3.downstream_analysis/",groups,"_",samples,"/statistical_tests/factors_chd_tests.csv", sep = ""),
          row.names = FALSE)

cat("\n\n... X.2) STATISTICAL TESTS BETWEEN INDIVIDUALS WITH CHD AND NO CHD WITHIN INDIVIDUALS WITH CVD... \n\n", file = stdout())

factors.chd.filt <- factors$group1[rownames(factors$group1)%in%MOFA.covariates.chd.filt$sample,]
# head(factors.chd.filt)
# dim(factors.chd.filt)

# Order
factors.chd.filt <- factors.chd.filt[match(MOFA.covariates.chd.filt$sample,rownames(factors.chd.filt)),]

# all.factors.res <- c()
# for (i in 1:30) {
#   test.factor.res <- testFactor(factors = factors.chd.filt, factor = i, covariable = "chd", group = "all", MOFA_samples_metadata = MOFA.covariates.chd.filt)
#   all.factors.res <- rbind(all.factors.res, test.factor.res)
# }

MOFA.covariates.chd.filt[1:20,c("sample","chd")]
...
testFeaturesFactor <- function(features.matrix, factor, omic) {
  ...
  for (i in 1:length(colnames(features.matrix))) {
    ...
    # Obtain any factorX_cov_date, because we only need the follow up date for the cox regression
    ...
    # Order by sample ID the features.matrix and factor.cov.dates
    factor.cov.dates <- factor.cov.dates[order(as.numeric(factor.cov.dates$shareid)),]
    
    cpg.mvals <- as.data.frame(features.matrix[,i])
    # print(head(cpg.mvals))
    # print(dim(cpg.mvals))
    colnames(cpg.mvals)[1] <- colnames(features.matrix)[i]
    cpgs.mvals <- cpg.mvals[order(as.numeric(rownames(cpg.mvals))),]
    # print(head(cpg.mvals))
    # print(dim(cpg.mvals))
    # Merge CpGs M values and followup data
    features.cox.df <- cbind(factor.cov.dates,cpgs.mvals)
    # print(head(features.cox.df))
    # print(dim(features.cox.df))
    ...
    if (omic == "mRNA") {
      colnames(features.cox.df)[25] <- "mRNA"
      label <- "Transcript expression"
    }
    else if (omic == "meth") {
      colnames(features.cox.df)[25] <- "cpg"
      label <- "CpG M value"
    }
    
    # Test only in females for factor 21
  
    if (factor == 21) {
      features.cox.df <- features.cox.df[features.cox.df$sex==2,]
    }
    ...
    # Cox regression
    mod.surv.cvd.1 <- coxph(Surv(cvdfollowup, cvd) ~
                            cpg +
                            CD8T +
                            CD4T +
                            NK +
                            ...
                            Gran +
                            sva1_METH,
                            data=features.cox.df)

    # Add Confidence Intervals
    mod.surv.cvd.1.df <- as.data.frame(summary(mod.surv.cvd.1)$coefficients)
    mod.surv.cvd.1.df$lower95 <- as.numeric(exp(confint(mod.surv.cvd.1)))[1:dim(mod.surv.cvd.1.df)[1]]
    mod.surv.cvd.1.df$upper95 <- as.numeric(exp(confint(mod.surv.cvd.1)))[(dim(mod.surv.cvd.1.df)[1]+1):(dim(mod.surv.cvd.1.df)[1]*2)]
    ...
    mod.surv.cvd.1.df$feature_name <- colnames(features.matrix)[i]
    # Reorder columns
    mod.surv.cvd.1.df <- mod.surv.cvd.1.df[, c(10,9,8,1,2,3,4,6,7,5)]
    #feature.name <- colnames(features.matrix)[i]
    #results.df <- data.frame(feature_name = feature.name, coef = mod.surv.cvd.1.df$coef, HR = , lower95 = , upper95 =, 
    #                         se_coeff = , p_value =, FDR_correction = NULL)
    heatmap.features.tests <- rbind(heatmap.features.tests, mod.surv.cvd.1.df)

  }
  heatmap.features.tests <- heatmap.features.tests[grep("cpg",rownames(heatmap.features.tests)),]
  rownames(heatmap.features.tests) <- NULL
...
meth.30features.heatmap[[1]]

cat("\nM matrix subset for that specific features\n", file = stdout())
MOFA.data <- get_data(MOFA.model)
M.matrix <- t(MOFA.data$meth$group1[rownames(MOFA.data$meth$group1)%in%meth.30features.heatmap[[1]],])
dim(M.matrix)
M.matrix[1:5,1:5]
...
# corrFeatures(M.matrix, factor = 1, omic = "meth")
...
meth.30features.heatmap[[2]]

cat("\nM matrix subset for that specific features\n", file = stdout())
MOFA.data <- get_data(MOFA.model)
M.matrix <- t(MOFA.data$meth$group1[rownames(MOFA.data$meth$group1)%in%meth.30features.heatmap[[2]],])
dim(M.matrix)
M.matrix[1:5,1:5]
...
# cat("\nCorrelations between methylation features\n", file = stdout())
# corrFeatures(M.matrix, factor = 2, omic = "meth")
...
meth.30features.heatmap <- get(meth.30features.heatmap.name)
cat("\n// Top 20 features from Factor of interest heatmap for Methylation //\n", file = stdout())
meth.30features.heatmap[[3]]

cat("\nM matrix subset for that specific features\n", file = stdout())
MOFA.data <- get_data(MOFA.model)
M.matrix <- t(MOFA.data$meth$group1[rownames(MOFA.data$meth$group1)%in%meth.30features.heatmap[[3]],])
dim(M.matrix)
M.matrix[1:5,1:5]

# Test only on females

cat("\n\nPerforming histogram, boxplot, t-test and Mann-Whitney-Wilcoxon test for each feature\n\n", file = stdout())
meth.heatmap.features.tests.3 <- testFeaturesFactor(features.matrix = M.matrix, factor = factor, omic = "meth")

cat("\nResults for methylation features tests\n", file = stdout())
meth.heatmap.features.tests.3

# cat("\nCorrelations between methylation features\n", file = stdout())
# corrFeatures(M.matrix, factor = 3, omic = "meth")
...
meth.30features.heatmap <- get(meth.30features.heatmap.name)
cat("\n// Top 20 features from Factor of interest heatmap for Methylation //\n", file = stdout())
meth.30features.heatmap[[4]]

cat("\nM matrix subset for that specific features\n", file = stdout())
MOFA.data <- get_data(MOFA.model)
M.matrix <- t(MOFA.data$meth$group1[rownames(MOFA.data$meth$group1)%in%meth.30features.heatmap[[4]],])
dim(M.matrix)
M.matrix[1:5,1:5]

cat("\n\nPerforming histogram, boxplot, t-test and Mann-Whitney-Wilcoxon test for each feature\n\n", file = stdout())
meth.heatmap.features.tests.4 <- testFeaturesFactor(features.matrix = M.matrix, factor = factor, omic = "meth")

cat("\nResults for methylation features tests\n", file = stdout())
meth.heatmap.features.tests.4

# cat("\nCorrelations between methylation features\n", file = stdout())
# corrFeatures(M.matrix, factor = 4, omic = "meth")
...
# p-values FDR adjust
meth.heatmap.features.tests.1$FDR_correction <- p.adjust(meth.heatmap.features.tests.1$P, method = "fdr")
meth.heatmap.features.tests.2$FDR_correction <- p.adjust(meth.heatmap.features.tests.2$P, method = "fdr")
meth.heatmap.features.tests.3$FDR_correction <- p.adjust(meth.heatmap.features.tests.3$P, method = "fdr")
meth.heatmap.features.tests.4$FDR_correction <- p.adjust(meth.heatmap.features.tests.4$P, method = "fdr")

total.heatmap.features.tests <- rbind(meth.heatmap.features.tests.1, meth.heatmap.features.tests.2,meth.heatmap.features.tests.3,meth.heatmap.features.tests.4)

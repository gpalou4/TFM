...
# 
# MOFA.model <- load_model("MOFA/2.training_model/MOFA_trained_nogroups_934.hdf5")
# MOFA.model
# 
...
##################################################################################################
####### Biomarker (Factor 14), cardiovascular outcome (CVD) and risk factors (covariables) #######
##################################################################################################
...
coxRegression <- function(biomarker) {
  ...
  filter.cov <- c("shareid","factor", "cvd", "age", "sex", "tot_chol", "hdl_chol", "sbp", "dbp", "glucose", "smoke")
  ...
  end.followup <- end.followup[,c("shareid","cvddate")]
  head(end.followup)
  dim(end.followup)
  ...
  factor.cov.dates$followup = factor.cov.dates$cvddate - factor.cov.dates$date8
  ...
  summary(factor.cov.dates[factor.cov.dates$cvd == 1, "followup"])
  cat("\nSummary of follow-up time for non-CVD individuals\n", file = stdout())
  summary(factor.cov.dates[factor.cov.dates$cvd == 0, "followup"])
  ...
  ##################################################################################################
  ### Association between biomarker and incidence of CVD (we need the time to event) ###
  ##################################### COX REGRESSION #############################################
  ##################################################################################################
...
write.table(Cstat.results,
            file=paste("MOFA/3.downstream_analysis/",groups,"_",samples,"/prediction/Cstat_results_female.csv", sep = ""),
            col.names = T,
            row.names = T,
            sep=",")
...
nri_km
write.table(nri_km,
            file=paste("MOFA/3.downstream_analysis/",groups,"_",samples,"/prediction/nri_km_female.csv", sep = ""),
            col.names = T,
            row.names = T,
            sep=",")
...
write.table(nricat,
            file=paste("MOFA/3.downstream_analysis/",groups,"_",samples,"/prediction/nri_clinical_female.csv", sep = ""),
            col.names = T,
            row.names = T,
            sep=",")
